...
GAME_URL = "https://game.rariaden-hub.ru/?v=1.0.2"
...
async def game(update: Update, context: CallbackContext) -> None:
    ...
    #keyboard = [[InlineKeyboardButton("Play Game", callback_data=GAME_SHORT_NAME)]]
    #reply_markup = InlineKeyboardMarkup(keyboard)
    #await update.message.reply_text('Нажмите кнопку ниже, чтобы сыграть в игру.', reply_markup=reply_markup)
...
async def play_game(update: Update, context: CallbackContext) -> None:
    ...
    # await query.answer()
    # reply_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Play", url=GAME_URL)]])
    # await update.get_bot().send_game(chat_id=query.message.chat_id, game_short_name=GAME_SHORT_NAME, reply_markup=reply_keyboard)
